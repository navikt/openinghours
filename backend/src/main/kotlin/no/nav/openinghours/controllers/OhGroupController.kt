...
class OhGroupController(
    ...
    @Operation(summary = "List empty opening hours groups (no rules or child groups) whose most recent activity is at least one year old, including groups still linked to a service.")
    @GetMapping("/outdated")
    fun getOutdated(): List<OhGroup> = service.findEmptyOutdated()

    @Operation(summary = "Delete all empty opening hours groups (no rules or child groups) whose most recent activity is at least one year old, including groups still linked to a service (their service link is removed too). Returns 409 listing the affected groups unless ?confirm=true is passed.")
