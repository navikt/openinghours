            ...
            childIds.map { id ->
                rulesById[id]?.let {
                    ResolvedRule(
                        name = it.name,
                        rule = it.rule,
                        displayHeader = it.header?.takeIf { header -> header.isNotBlank() },
                        displayText = it.text?.takeIf { text -> text.isNotBlank() },
                        onlyShowForNavEmployees = it.onlyShowForNavEmployees,
                        redDay = it.redDay,
                        unstableOpeningHours = it.unstableOpeningHours,
                    )
                }
